...
def parse_and_sort_entries(content):
    """Парсит и сортирует записи по номеру."""
    entries = []
    pattern = r'\[t(\d+)(?:-[^]]+)?\](.*?)\[/t\1\]'
    ...
    # Возвращаем только отсортированный текст
    sorted_entries = [text for _, text in entries]
...
def pack_txb(input_txb, input_txt, output_txb):
    ...
    # Чтение и сортировка текста из файла .txt
    with open(input_txt, 'r', encoding='utf-8') as txt_file:
        content = txt_file.read()
        text_entries = parse_and_sort_entries(content)
